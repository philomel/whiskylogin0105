...
const MyAccount: React.FC<MyAccountProps> = ({ section }) => {
  ...
  const renderHeader = (title: string, subtitle?: string) => (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            ...
            {subtitle && <p className="text-whisky-muted mt-1">{subtitle}</p>}
        </div>
        
        <div className="flex items-center gap-3">
             {section === 'my-nfts' && (
                <div className="flex bg-white rounded-lg border border-whisky-border p-1 shadow-sm">
                    <button onClick={() => setViewMode('GRID')} className={`p-2 rounded ${viewMode === 'GRID' ? 'bg-whisky-button text-whisky-button-text' : 'text-whisky-muted hover:text-whisky-dark'}`}><Grid className="w-4 h-4" /></button>
                    <button onClick={() => setViewMode('LIST')} className={`p-2 rounded ${viewMode === 'LIST' ? 'bg-whisky-button text-whisky-button-text' : 'text-whisky-muted hover:text-whisky-dark'}`}><List className="w-4 h-4" /></button>
                </div>
             )}
        </div>
    </div>
  ...
  const renderMyNFTs = () => (
    <>
        {/* TWR Token Balance Header Section */}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
            <div className="flex gap-2 overflow-x-auto pb-2 w-full md:w-auto">
                {['All (32)', 'Not Listed (20)', 'Listed (12)'].map(tab => (
                    <button key={tab} className="px-4 py-2 bg-white border border-whisky-border rounded-full text-sm text-whisky-muted hover:text-whisky-dark whitespace-nowrap shadow-sm">
                        {tab}
                    </button>
                ))}
            </div>
            <button className="flex items-center gap-2 text-sm text-whisky-muted border border-whisky-border px-4 py-2 rounded-lg hover:text-whisky-dark bg-white shadow-sm font-sans">
                    ...
                    <div key={nft.id} className="relative group">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur border-t border-whisky-border translate-y-full group-hover:translate-y-0 transition-transform flex justify-between items-center z-20 rounded-b-xl shadow-lg">
                            ...
                            <div className="flex gap-2">
                                <Link to={`/profile/list-nft/${nft.id}`} className="text-xs bg-whisky-button text-whisky-button-text px-3 py-1.5 rounded font-bold hover:opacity-90 transition-opacity">List</Link>
                                <Link to={`/profile/transfer/${nft.id}`} className="text-xs border border-whisky-border text-whisky-muted px-3 py-1.5 rounded hover:bg-gray-50 transition-colors">Transfer</Link>
                            </div>
                        </div>
                    </div>
                ))}
  ...
  const renderContent = () => {
    switch(section) {
        ...
        default: return <div className="p-20 text-center text-whisky-muted">Loading content...</div>;
    }
  };
